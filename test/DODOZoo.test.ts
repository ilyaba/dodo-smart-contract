...
describe("DODO ZOO", () => {
  ...
  describe("Breed new dodo", () => {
    ...
    it("breed new dodo", async () => {
      ...
      assert.strictEqual(await newDODO.methods._BASE_TOKEN_().call(), newBase.options.address)
      assert.strictEqual(await newDODO.methods._QUOTE_TOKEN_().call(), newQuote.options.address)
    ...
    it("dodo register control flow", async () => {
      await ctx.DODOZoo.methods.removeDODO(ctx.DODO.options.address).send(ctx.sendParam(ctx.Deployer))
      assert.strictEqual(await ctx.DODOZoo.methods.getDODO(ctx.BASE.options.address, ctx.QUOTE.options.address).call(), "0x0000000000000000000000000000000000000000")
      await truffleAssert.reverts(
      ...
      assert.strictEqual(await ctx.DODOZoo.methods.getDODO(ctx.BASE.options.address, ctx.QUOTE.options.address).call(), ctx.DODO.options.address)
      await truffleAssert.reverts(
