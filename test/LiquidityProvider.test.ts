...
describe("LiquidityProvider", () => {
  ...
  describe("R equals to ONE", () => {
    it("multi lp deposit & withdraw", async () => {
      ...
      await logGas(
        ctx.AkwaPool.methods.depositBase(decimalStr("10")),
        ctx.sendParam(lp1),
        "deposit base"
      );
      assert.strictEqual(
      ...
      await logGas(
        ctx.AkwaPool.methods.depositQuote(decimalStr("1000")),
        ctx.sendParam(lp1),
        "deposit quote"
      );
      assert.strictEqual(
      ...
      assert.strictEqual(await ctx.AkwaPool.methods.getLpBaseBalance(lp1).call(), "0");
      assert.strictEqual(
        await ctx.BASE.methods.balanceOf(lp1).call(),
        decimalStr("100")
      );
      await ctx.AkwaPool.methods.withdrawAllQuote().send(ctx.sendParam(lp1));
      assert.strictEqual(await ctx.AkwaPool.methods.getLpQuoteBalance(lp1).call(), "0");
      assert.strictEqual(
        await ctx.QUOTE.methods.balanceOf(lp1).call(),
        decimalStr("10000")
      );
  ...
  describe("R is ABOVE ONE", () => {
    it("deposit", async () => {
      ...
      assert.strictEqual(
        await ctx.AkwaPool.methods.getWithdrawBasePenalty(decimalStr("5")).call(),
        "228507420047606093"
      );
      assert.strictEqual(
        await ctx.AkwaPool.methods
    ...
    it("withdraw", async () => {
      ...
      assert.strictEqual(
        await ctx.AkwaPool.methods.getWithdrawBasePenalty(decimalStr("4")).call(),
        "1065045389392391665"
      );
      assert.strictEqual(
        await ctx.AkwaPool.methods
  ...
  describe("Edge cases", () => {
    it("single side deposit", async () => {
      ...
      await ctx.AkwaPool.methods.depositQuote("1").send(ctx.sendParam(lp2));

      assert.strictEqual(
        await ctx.AkwaPool.methods.getQuoteCapitalBalanceOf(lp2).call(),
        "1103903610832497493"
      );
    });
    ...
    it("single side deposit & lp deposit when R isn't equal to ONE", async () => {
      ...
      assert.strictEqual(await ctx.AkwaPool.methods.getLpQuoteBalance(lp2).call(), "1");
    });
    ...
    it("single side deposit (quote) & oracle change introduces loss", async () => {
      ...
      await ctx.AkwaPool.methods.depositBase("1").send(ctx.sendParam(lp2));
      assert.strictEqual(
        await ctx.AkwaPool.methods.getBaseCapitalBalanceOf(lp2).call(),
        "10247647352975730"
      );
      assert.strictEqual(
        await ctx.AkwaPool.methods.getLpBaseBalance(lp2).call(),
  ...
  describe("Revert cases", () => {
    it("withdraw base amount exceeds DODO balance", async () => {
      ...
      await truffleAssert.reverts(
        ctx.AkwaPool.methods.withdrawBase(decimalStr("6")).send(ctx.sendParam(lp1)),
    ...
    it("withdraw base could not afford penalty", async () => {
      ...
      await truffleAssert.reverts(
        ctx.AkwaPool.methods.getWithdrawBasePenalty(decimalStr("10")).call(),
        "AKWA_POOL_BASE_BALANCE_NOT_ENOUGH"
      );
    });
    ...
    it("withdraw quote could not afford penalty", async () => {
      ...
      await truffleAssert.reverts(
        ctx.AkwaPool.methods.getWithdrawQuotePenalty(decimalStr("1000")).call(),
        "AKWA_POOL_QUOTE_BALANCE_NOT_ENOUGH"
      );
    });
    ...
    it("withdraw when there is no lp", async () => {
      await truffleAssert.reverts(
        ctx.AkwaPool.methods.withdrawBase(decimalStr("1")).send(ctx.sendParam(lp1)),
        "NO_BASE_LP"
      );
      await truffleAssert.reverts(
        ctx.AkwaPool.methods
